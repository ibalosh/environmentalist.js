...
class EnvHandler {
    ...
    takeEnvironment(environment, user_name, user_id, channel_id) {
        ...
        if (!this.environments.environmentExists(environment)) {
            return this.generateResponse("Environment *" + environment + "* doesn't exist. Available environments are *" + this.environments.getEnvironmentNames().join(", ") + "*", 403)
        }
    ...
    addEnvironment(environment) {
        ...
        if (environment === undefined) {
            return this.generateResponse('Invalid content provided.', 403)
        }
        // environment exists
        else if (this.environments.environmentExists(environment)) {
            return this.generateResponse('Environment already exists.', 403)
        }
        // add environment
        else {
    ...
    deleteEnvironment(environment) {
        ...
        // environment doesn't exist
            ...
            return this.generateResponse('Environment doesnt exist.', 403)
        }
        // delete environment
        else {
